...
public class CopyMoveLitmusTest extends AbstractLitmusTest {
    ...
    public void test_02_copy_init() throws Exception {
        for (final String test : Arrays.asList("02-01.xml", "02-02.xml")) {
            ...
            Assert.assertEquals("status must match: " + test, Status.CREATED, response.getStatus());
        }
    }
    ...
    public void test_04_copy_overwrite() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.PRECONDITION_FAILED, copy_response.getStatus());

        for (final String test : Arrays.asList("04-02.xml", "04-03.xml")) {
            final TestResponse response = execute(new File(ROOT, test));
            Assert.assertEquals("status must match: " + test, Status.NO_CONTENT, response.getStatus());
        }
    }
    ...
    public void test_05_copy_nodestcoll() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.CONFLICT, response.getStatus());
    }
    ...
    public void test_06_copy_cleanup() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.NOT_FOUND, response.getStatus());
    }
    ...
    public void test_07_copy_coll() throws Exception {
        ...
        for (final String test : Arrays.asList("07-13.xml", "07-14.xml")) {
            final TestResponse response = execute(new File(ROOT, test));
            Assert.assertEquals("status must match: " + test, Status.NOT_FOUND, response.getStatus());
        }
        ...
        Assert.assertEquals("status must match", Status.PRECONDITION_FAILED, copy_on_exisiting_response.getStatus());
    ...
    public void test_08_copy_shallow() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.NOT_FOUND, delete_desitination_response.getStatus());

        final TestResponse copy_response = execute(new File(ROOT, "08-04.xml"));
        Assert.assertEquals("status must match", Status.CREATED, copy_response.getStatus());

        final TestResponse delete_source_response = execute(new File(ROOT, "08-05.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, delete_source_response.getStatus());

        final TestResponse delete_non_exisiting_response = execute(new File(ROOT, "08-06.xml"));
        Assert.assertEquals("status must match", Status.NOT_FOUND, delete_non_exisiting_response.getStatus());

        final TestResponse cleanup_response = execute(new File(ROOT, "08-07.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, cleanup_response.getStatus());
    }
    ...
    public void test_09_move() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.CREATED, mkcol_response.getStatus());

        final TestResponse move_new_response = execute(new File(ROOT, "09-04.xml"));
        Assert.assertEquals("status must match", Status.CREATED, move_new_response.getStatus());

        final TestResponse move_exisiting_response = execute(new File(ROOT, "09-05.xml"));
        Assert.assertEquals("status must match", Status.PRECONDITION_FAILED, move_exisiting_response.getStatus());
    ...
    public void test_10_move_coll() throws Exception {
        ...
        Assert.assertEquals("status must match", Status.CREATED, copy1_response.getStatus());
        final TestResponse move1_response = execute(new File(ROOT, "10-15.xml"));
        Assert.assertEquals("status must match", Status.CREATED, move1_response.getStatus());
        final TestResponse move2_response = execute(new File(ROOT, "10-16.xml"));
        Assert.assertEquals("status must match", Status.PRECONDITION_FAILED, move2_response.getStatus());
        final TestResponse move3_response = execute(new File(ROOT, "10-17.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, move3_response.getStatus());
        final TestResponse copy2_response = execute(new File(ROOT, "10-18.xml"));
        Assert.assertEquals("status must match", Status.CREATED, copy2_response.getStatus());
        ...
        for (final String test : Arrays.asList("10-25.xml", "10-26.xml", "10-27.xml", "10-28.xml", "10-29.xml")) {
            final TestResponse response = execute(new File(ROOT, test));
            Assert.assertEquals("status must match: " + test + test, Status.NO_CONTENT, response.getStatus());
        }

        final TestResponse move_sparse_response = execute(new File(ROOT, "10-30.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, move_sparse_response.getStatus());
    }

    @Test
    public void test_11_move_cleanup() throws Exception {
        final TestResponse delete1_response = execute(new File(ROOT, "11-01.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, delete1_response.getStatus());

        final TestResponse delete2_response = execute(new File(ROOT, "11-02.xml"));
        Assert.assertEquals("status must match", Status.NOT_FOUND, delete2_response.getStatus());

        final TestResponse delete3_response = execute(new File(ROOT, "11-03.xml"));
        Assert.assertEquals("status must match", Status.NO_CONTENT, delete3_response.getStatus());
    }
}
